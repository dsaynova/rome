...
def trace_with_patch(
    ...
    if answers_t is not None:
        probs = torch.softmax(outputs_exp.logits[1:, -1, :], dim=1).mean(dim=0)
        probs = probs[answers_t]
    else:
        # HACK: do not average over the different perturbed input variants
...
def calculate_hidden_flow(
    ...
    """
    answer_t = mt.tokenizer.encode(expect, add_special_tokens=False)
...
def plot_normalized_trace_heatmap_from_values(differences, vmin, vmax, bartitle, kind, window, labels,
                                   savepdf, title, xlabel, modelname):
    with plt.rc_context():
        fig, ax = plt.subplots(figsize=(3.5, 2), dpi=200)
        h = ax.pcolor(
        ...
        if xlabel is not None:
            ax.set_xlabel(xlabel)
        elif bartitle is not None:
            # The following should be cb.ax.set_xlabel, but this is broken in matplotlib 3.5.1.
            cb.ax.set_title(bartitle, y=-0.16, fontsize=10)
        if savepdf:
            os.makedirs(os.path.dirname(savepdf), exist_ok=True)
            plt.savefig(savepdf, bbox_inches="tight")
            plt.close()
